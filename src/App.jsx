...
const PROFILE = {
  ...
  tagline: 'Python APIs, Django & Flask services, and GenAI pipelines that hold up in production.',
...
const EXPERIENCE = [
  {
    ...
    status: 'current',
    points: [
      'Built an Azure Durable Functions pipeline to process 20,000+ pages in parallel, cutting document turnaround time significantly.',
      'Built and shipped backend APIs in Flask integrated with a React.js frontend, owning end-to-end feature delivery.',
      'Integrated Azure AI Services and OpenAI APIs into GenAI workflows, improving response reliability and cutting manual processing steps.',
    ],
  },
  {
    role: 'Backend Developer',
    ...
    status: 'done',
    points: [
      'Designed and built RESTful APIs using Django REST Framework for a client-facing product, following clean architecture principles.',
      'Containerized all backend services with Docker, eliminating environment inconsistencies across dev and production.',
    ],
  },
  {
    role: 'Summer Intern',
...
const PROJECTS = [
  {
    ...
    version: 'v3.0.0',
    stack: ['Django', 'LangChain', 'Qdrant', 'React', 'PostgreSQL', 'Redis', 'Docker'],
    description:
  ...
  },
  {
    ...
    description: 'A recipe manager with a clean REST architecture and full CRUD support.',
    points: [
...
const SKILLS = [
  ...
  { group: 'backend', items: ['Django', 'Django REST Framework', 'Flask', 'REST APIs', 'LangChain', 'Celery', 'Redis'] },
  { group: 'databases', items: ['SQL', 'PostgreSQL', 'Qdrant'] },
  { group: 'cloud & devops', items: ['Azure Functions', 'Azure Durable Functions', 'Azure AI Services', 'Docker', 'Git'] },
  { group: 'genai', items: ['OpenAI API', 'LangChain', 'RAG Pipelines', 'Azure AI Services'] },
]
...
const JSON_LINES = (years) => [
  ...
  { key: 'stack', value: '["Python", "Django", "Flask", "GenAI"]', type: 'raw' },
...
function JsonLine({ line, isLast }) {
  ...
  const valueNode =
    line.type === 's' ? <span className="s">"{line.value}"</span>
    : line.type === 'n' ? <span className="n">{line.value}</span>
    : <span className="s">{line.value}</span>
  return (
    <div>
      &nbsp;&nbsp;<span className="k">"{line.key}"</span>: {valueNode}{!isLast && ','}
    </div>
  )
}
  ...
  return (
    <section id="top" className="hero">
          ...
          <div className="terminal-body">
            <div className="terminal-line">
              ...
              <span className="cmd">
                ...
                <TypedLine text="--pretty" onDone={() => setShowResponse(true)} />
              </span>
            </div>
              ...
              <div className="response fade-in">
                ...
                <pre className="json">
                  {jsonLines.map((line, i) => (
                    <JsonLine key={i} line={line} isLast={i === jsonLines.length - 2} />
                  ))}
                </pre>
              </div>
            )}
      ...
      <Reveal delay={150} className="hero-copy">
        <h1>
          {PROFILE.name}
          <span className="hero-role">/{PROFILE.role.toLowerCase().replace(/\s+/g, '-')}</span>
        </h1>
  ...
  return (
    <section id="about" className="section">
      <Reveal>
        ...
        <h2>What I build</h2>
        <p className="lede">
          I'm a backend developer with {years}+ years of hands-on experience shipping Python APIs
          and cloud-integrated systems. Most of my recent work sits at the intersection of
          traditional backend engineering and GenAI — building the plumbing that makes LLM
          features reliable in production, not just in a notebook. At IBM, that meant a
          document pipeline handling 20,000+ pages; on my own projects, it's meant retrieval
          pipelines, rate limiting, and auth done properly.
        </p>
      </Reveal>
    </section>
  )
        ...
        {EXPERIENCE.map((job, i) => (
          <Reveal key={job.company} delay={i * 100} className="timeline-item">
            <div className="timeline-marker">
              <span className={`timeline-dot ${job.status === 'current' ? 'timeline-dot-live' : ''}`} />
  ...
  return (
    <Reveal delay={index * 100} className="project-card">
      ...
      <div className="project-links">
        <a href={project.github} target="_blank" rel="noreferrer noopener">source →</a>
        {project.live && (
          <a href={project.live} target="_blank" rel="noreferrer noopener" className="project-live">
            live demo →
          </a>
        )}
      </div>
  ...
  return (
    <section className="section section-tight">
      <Reveal>
        ...
        <p className="lede">{EDUCATION.school} · {EDUCATION.period}</p>
      </Reveal>
    </section>
  )
}
  ...
  return (
    <section id="contact" className="section contact">
      <Reveal>
        ...
        <p className="lede">
          Open to backend and GenAI roles. The fastest way to reach me is email — I check it daily.
        </p>
        <div className="contact-grid">
          ...
          <a className="contact-card" href={PROFILE.github} target="_blank" rel="noreferrer noopener">
            <span className="contact-label">github</span>
            <span className="contact-value">/sameer2002ms</span>
          </a>
          <a className="contact-card" href={PROFILE.leetcode} target="_blank" rel="noreferrer noopener">
